...
function toHtml(data) {
  ...
  if (typeof data === "object") {
    const rows = Object.entries(data).map(([k, v]) => `<tr><th>${k}</th><td>${v}</td></tr>`).join("");
...
app.get("/health/ready", async (req, res) => {
  ...
  try {
    ...
    respond(req, res, 500, { status: "error", message: "Database connection failed", detail: err.message });
  } finally {
...
app.get("/tasks", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query("SELECT id, title, status, created_at FROM tasks ORDER BY created_at DESC");
    respond(req, res, 200, rows);
  } catch (err) {
    console.error(err);
    respond(req, res, 500, { error: "Failed to fetch tasks", detail: err.message });
  } finally {
    if (conn) conn.release();
  }
});
...
app.post("/tasks", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const result = await conn.query("INSERT INTO tasks (title, status) VALUES (?, 'pending')", [title.trim()]);
    const [task] = await conn.query("SELECT id, title, status, created_at FROM tasks WHERE id = ?", [result.insertId]);
    respond(req, res, 201, task);
  } catch (err) {
    console.error(err);
    respond(req, res, 500, { error: "Failed to create task", detail: err.message });
  } finally {
    if (conn) conn.release();
  }
});
...
app.post("/tasks/:id/done", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const result = await conn.query("UPDATE tasks SET status = 'done' WHERE id = ?", [id]);
    if (result.affectedRows === 0) {
      return respond(req, res, 404, { error: "Task not found" });
    }
    const [task] = await conn.query("SELECT id, title, status, created_at FROM tasks WHERE id = ?", [id]);
    respond(req, res, 200, task);
  } catch (err) {
    console.error(err);
    respond(req, res, 500, { error: "Failed to update task", detail: err.message });
  } finally {
    if (conn) conn.release();
  }
});
